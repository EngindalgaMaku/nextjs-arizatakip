...
export default function LocationPropertiesModal({ location, onClose }: LocationPropertiesModalProps) {
  if (!location || !location.properties || Object.keys(location.properties).length === 0) {
    return null; // Don't render if no location or properties
  }
  ...
  return (
    <div className="fixed inset-0 bg-gray-500 bg-opacity-75 z-50 flex justify-center items-center p-4">
      <div className="relative bg-white p-6 rounded-lg shadow-xl max-w-lg w-full">
        ...
        <p className="text-lg font-semibold text-gray-700 mb-4 border-b pb-2 text-center">{location.name}</p>
